...
def get_spectra():
    ...
    _, nr, kab, kcar, kant, kbrown, kw, km = torch.tensor(
        np.loadtxt(BytesIO(prospect_d_spectraf), unpack=True),dtype=torch.float32,device='cuda')
    ...
    _, nr, kab, kcar, kant, kbrown, kw, km, kprot, kcbc = torch.tensor(
        np.loadtxt(BytesIO(prospect_pro_spectraf), unpack=True),dtype=torch.float32,device='cuda')
    ...
    nr, kab, kcar, kbrown, kw, km = torch.tensor(
        np.loadtxt(BytesIO(prospect_5_spectraf), unpack=True),dtype=torch.float32,device='cuda')
    prospect_5_spectra = Prospect5Spectra(nr, kab, kcar, kbrown, kw, km)
    # SOIL
    soil_spectraf = pkgutil.get_data("prosail", "soil_reflectance.txt")
    rsoil1, rsoil2 = torch.tensor(
        np.loadtxt(BytesIO(soil_spectraf), unpack=True),dtype=torch.float32,device='cuda')
    soil_spectra = SoilSpectra(rsoil1, rsoil2)
    # LIGHT
    light_spectraf = pkgutil.get_data("prosail", "light_spectra.txt")
    es, ed = torch.tensor(np.loadtxt(BytesIO(light_spectraf), unpack=True),dtype=torch.float32,device='cuda')
